      ...
      chats?.map((chat: any) => {
        return {
          ...
          isIgnore:
            ...
            toUpper(
              // @ts-ignore
              JSON.parse(localStorage.getItem("currentUser"))?.startingMessage
            ),
  ...
  return map(sortedChats, (sortedChat, index) => {
    if (
      ...
      return {
        ...
        payload: {
          ...sortedChat?.payload,
          buttonChoices: map(sortedChat?.payload?.buttonChoices, (choice) => {
            return {
              ...choice,
              active: choice?.key == sortedChats?.[index + 1]?.payload?.text,
            };
          }),
        },
