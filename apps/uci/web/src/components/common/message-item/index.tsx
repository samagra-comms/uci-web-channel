  ...
  switch (type) {
      ...
      return (
        <>
          {content?.data?.position === "left" && (
            ...
            >
              <Image src={configData.botImage} alt="botImage" style={{ borderRadius: "50%" }} />
            </div>
          )}
          <Bubble type="image">
            <div style={{ padding: configData.video.style.padding }}>
              <Video
                cover={configData.video.cover}
                src={url}
              />
              ...
              >
                <span style={{ color: configData.video.style.color, fontSize: configData.video.style.fontSize }}>
                  {moment
                    .utc(
                      content?.data?.sentTimestamp ||
                      content?.data?.repliedTimestamp
                ...
                <span>
                  {content?.data?.position === "left" && (
                    <FontAwesomeIcon
                      icon={faStar}
                      onClick={(): void => onLongPress(content)}
                      color={isStarred ? configData.video.starColor.starred : configData.video.starColor.default}
                    />
                  )}
                </span>
              </div>
    ...
    case "options": {
      ...
      return (
        <>
          <div
            style={{ width: configData.options.style.width, marginRight: configData.options.style.marginRight, textAlign: configData.options.style.textAlign as 'left' | 'right' | 'center' }}
          >
            <Image src={configData.botImage} alt="userImage" style={{ borderRadius: "50%" }} />
          </div>
          <Bubble type="text">
            <div style={{ display: "flex" }}>
              <span style={{ fontSize: configData.options.style.fontSize }}>{content.text}</span>
            </div>
              ...
              <span>
                {content?.data?.position === "left" && (
                  <FontAwesomeIcon
                    icon={faStar}
                    onClick={(): void => onLongPress(content)}
                    color={isStarred ? configData.options.starColor.starred : configData.options.starColor.default}
                  />
                )}
              </span>
            </div>
