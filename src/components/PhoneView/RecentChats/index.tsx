...
const RecentChats: React.FC<recentChatsProps> = ({ allUsers }) => {
	...
	const StarredViewHandler = (): void => {
		...
		window && window?.androidInteract?.onEvent('nl-chatbotscreen-chatbot_starmessagesearch', JSON.stringify({ timestamp: moment().valueOf() }));
		window && window?.androidInteract?.log(`nl-chatbotscreen-chatbot_starmessagesearch event: ${JSON.stringify({ timestamp: moment().valueOf() })}`);
	};

	// toChangeCurrentUser
	console.log('qwerst12:', { botToFocus, allUsers });
	window && window?.androidInteract?.log(`botToFocus:${botToFocus}`);
	useEffect(() => {
		try {
			if (botToFocus) {
				const bot = find(allUsers, { id: botToFocus });
				console.log('qwerst12', { bot });
				if (bot) {
					localStorage.removeItem('botToFocus');
					window && window?.androidInteract?.log(`removing botToFocus:${botToFocus}`);
							...
							<>
								{(allUsers ?? [])?.map((user, index) => (
									<div
										onClick={(): void => {
											try {
												window && window?.androidInteract?.onEvent('nl-chatbotscreen-chatbot-interactions', JSON.stringify({ botId: user.id, timestamp: moment().valueOf() }));
												window && window?.androidInteract?.log(`nl-chatbotscreen-chatbot-interactions event: ${JSON.stringify({ botId: user.id, timestamp: moment().valueOf() })}`);
											} catch (err) { 
												window && window?.androidInteract?.log(`error in opening the bot:${JSON.stringify(err)}`);
											}
										}}>
										<ChatItem
											key={index}
